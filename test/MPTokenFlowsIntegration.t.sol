 ...
 */
contract MPTokenFlowsIntegrationTest is BaseTest {
    // Contracts
    // Using factory from BaseTest (MiningPoolFactoryCore)
    MiningPoolDAOCore poolCore;
    RewardHandler poolRewards;
    RedemptionHandler poolRedemption;
    PoolMpToken mpToken;
    ...
    function setUp() public override {
        super.setUp(); // Call base setup first to initialize factory

        // All infrastructure already deployed in BaseTest
        // Calculator registry already has pplnsCalculator registered
        // Just deploy and register additional FPPS calculator if needed
        calculator = new FPPSCalculator();

        // Register FPPS calculator (as test contract is the admin)
        ...
        // Grant POOL_MANAGER_ROLE to admin
        // Test contract is the admin of factory from BaseTest
        factory.grantRole(factory.POOL_MANAGER_ROLE(), admin);

        // Grant ADMIN_ROLE to admin for MultiPoolDAO operations
        multiPoolDAO.grantRole(multiPoolDAO.ADMIN_ROLE(), admin);

        vm.startPrank(admin);
        // Dependencies already set in BaseTest
        vm.stopPrank();

        // Create S-token for BTC using correct constructor (test contract as admin)
        sTokenBTC = new PoolMpToken(
            "Synthetic Bitcoin",       // name
            "sBTC",                    // symbol
            address(this),             // Use test contract as minter initially
            false                      // restrictedMp (not restricted)
        );

        // Grant minter and burner role to MultiPoolDAO after creation (test contract has admin role)
        sTokenBTC.grantRole(sTokenBTC.MINTER_ROLE(), address(multiPoolDAO));
        sTokenBTC.grantRole(sTokenBTC.BURNER_ROLE(), address(multiPoolDAO));

        // Setup network in MultiPoolDAO (requires admin role from initialization)
    ...
    function testFlow1_BitcoinToMPTokens() public {
        ...
        poolCore = new MiningPoolDAOCore();
        poolCore.initialize(
            address(spv),
            address(frost),
            address(calculatorRegistry), // calculatorRegistry
            address(aggregator),   // aggregator
            address(validator),    // validator
            address(oracleRegistry),        // oracleRegistry
        ...
        // Note: Pool token is set during pool initialization, not separately

        // Deploy Rewards with all required parameters
        poolRewards = new RewardHandler();
        // Note: RewardHandler doesn't have an initialize function in the refactored version
        // poolCore.setRewardsContract(address(poolRewards)); // Also no setRewardsContract method

        // Deploy Redemption contract
        poolRedemption = new RedemptionHandler();
        // Note: RedemptionHandler doesn't have an initialize function in the refactored version

        // Extensions contract removed in refactor
    ...
    function testFlow3_MPTokensToSTokens() public {
        ...
        // For simplified test, burn S-tokens directly as multiPoolDAO (which has BURNER_ROLE)
        vm.stopPrank();
        vm.startPrank(address(multiPoolDAO));
