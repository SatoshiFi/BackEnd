 ...
 */
contract SPVValidationTest is BaseTest {
    MockSPVContract mockSpv; // Using different name since spv is in BaseTest
    ...
    function setUp() public override {
        ...
        mockSpv = new MockSPVContract();

        // Use mockSpv as the main spv for tests
        spv = SPVContract(address(mockSpv));
    ...
    function testFullSPVToMPTokenFlow() public {
        ...
        // Create a temporary mpToken for this test if not already created
        if (address(mpToken) == address(0)) {
            mpToken = new PoolMpToken("Test MP Token", "mpBTC", address(this), false);
        }
        // Always grant MINTER_ROLE to test contract to simulate pool minting
        // First check who is the admin of the token
        if (!mpToken.hasRole(mpToken.DEFAULT_ADMIN_ROLE(), address(this))) {
            // If we're not admin, we can't grant roles, so recreate the token
            mpToken = new PoolMpToken("Test MP Token", "mpBTC", address(this), false);
        }
        // Now grant MINTER_ROLE if needed
        if (!mpToken.hasRole(mpToken.MINTER_ROLE(), address(this))) {
            mpToken.grantRole(mpToken.MINTER_ROLE(), address(this));
        }
        ...
        // Since poolCore is not initialized and we're just simulating,
        // we'll mint from the test contract which has MINTER_ROLE
