...
import "./BaseTest.sol";
import "../contracts/src/interfaces/IPoolMpToken.sol";
...
contract RefactoredSystemTest is BaseRefactoredTest {
    ...
    function testRewardRegistration() public {
        ...
        // Grant role to test contract from deployer
    ...
    function testRedemptionRequest() public {
        // First need MP tokens - mint some to test contract
        IPoolMpToken mpTokenContract = IPoolMpToken(mpToken);

        // Grant MINTER_ROLE to this test contract temporarily
        vm.startPrank(poolAddress);
        mpTokenContract.grantRole(mpTokenContract.MINTER_ROLE(), address(this));
        vm.stopPrank();

        // Mint MP tokens to test contract
        mpTokenContract.mint(address(this), 500000);
    ...
    function testRedemptionConfirmation() public {
        ...
        // Grant confirmer role from deployer
        vm.startPrank(address(deployer));
    ...
    function testPayoutScriptSetting() public {
        // The deployer contract has all roles, including ADMIN_ROLE
        // Let's grant ourselves ADMIN_ROLE from the deployer
        vm.startPrank(address(deployer));
        pool.grantRole(pool.ADMIN_ROLE(), address(this));
        vm.stopPrank();

        // Now we can set payout script
    ...
    function testSPVVerification() public {
        // Create a valid 80-byte test block header
        ...
        bytes32 blockHash = bytes32(sha256(abi.encodePacked(sha256(header))));
    ...
    function testFullRewardCycle() public {
        ...
        // 2. Grant necessary roles from deployer
        vm.startPrank(address(deployer));
        pool.grantRole(pool.POOL_MANAGER_ROLE(), address(this));
        pool.grantRole(pool.CONFIRMER_ROLE(), address(this));
        vm.stopPrank();
        ...
        // 5. Mint tokens to test contract for redemption test
        IPoolMpToken mpTokenContract = IPoolMpToken(mpToken);
        vm.startPrank(poolAddress);
        mpTokenContract.grantRole(mpTokenContract.MINTER_ROLE(), address(this));
        vm.stopPrank();
        mpTokenContract.mint(address(this), 5000000);
