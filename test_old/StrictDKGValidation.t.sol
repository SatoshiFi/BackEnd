...
contract StrictDKGValidationTest is Test {
    // Contracts
    ...
    function setUp() public {
        ...
        // Deploy factory with dependencies
        factory = new MiningPoolFactory();

        SPVContract spv = new SPVContract();
        CalculatorRegistry calcReg = new CalculatorRegistry(admin, address(factory));
        StratumOracleRegistry oracleReg = new StratumOracleRegistry(admin);
        StratumDataAggregator aggregator = new StratumDataAggregator(admin, address(oracleReg));
        StratumDataValidator validator = new StratumDataValidator(admin, address(oracleReg));
        PoolTokenFactory tokenFactory = new PoolTokenFactory(admin);
        MultiPoolDAO multiDAO = new MultiPoolDAO();
        ...
        factory.setOptionalDependencies(address(0), address(membershipNFT), address(0));

        // Setup calculator
        FPPSCalculator calc = new FPPSCalculator();
        calcReg.authorizeAuthor(admin, true);
        uint256 calcId = calcReg.registerCalculator(
            address(calc),
            CalculatorRegistry.SchemeType.FPPS,
            "FPPS", "FPPS", "1.0.0", 100000
        );
        calcReg.whitelistCalculator(calcId, true);
    ...
    function testFactoryMintsNFTsOnlyToParticipants() public {
        ...
        // Check NFT balances AFTER pool creation
        for (uint i = 0; i < participants.length; i++) {
    ...
    function testCannotCreatePoolFromUnfinalizedSession() public {
        ...
        // Try to create pool WITHOUT completing DKG
