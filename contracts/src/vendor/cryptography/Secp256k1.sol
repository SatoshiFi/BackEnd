 ...
 *      - https://github.com/ethereum/go-ethereum/blob/v1.14.12/crypto/secp256k1/curve.go#L282
 */
library Secp256k1 {
    /**
    ...
    /**
     * @dev Compute y coordinate from x and parity flag.
     * @param x X coordinate.
     * @param yIsOdd parity (true -> odd, false -> even).
     * @return y Y coordinate such that (x, y) is on the curve.
     *
     * Note: This function uses Secp256k1Arithmetic.decompressToAffinePoint which
     * computes modular square root via modexp. It allocates a small memory region
     * for the exponentiation call.
     */
    function calculateY(uint256 x, bool yIsOdd) internal view returns (uint256 y) {
        require(x < P, "x out of range");
        // decompress uses compressed y: 2 (even) or 3 (odd)
        uint256 yCompressed = yIsOdd ? 3 : 2;
        // allocate small memory region expected by decompressToAffinePoint
        uint256 memPtr = Memory.allocate(0xC0); // 192 bytes — matches other uses
        ( , y) = Secp256k1Arithmetic.decompressToAffinePoint(memPtr, x, yCompressed);
        // sanity: ensure point lies on curve
        require(isOnCurve(x, y), "decompressed not on curve");
    }
    ...
    // в конце библиотеки Secp256k1

    /// @dev Вспомогательно: "поднять" x-only точку с четным y (BIP-340).
    function deriveY(uint256 x) internal view returns (uint256 y) {
        // Эквивалент calculateY(x, false)
        y = calculateY(x, false);
    }
}
